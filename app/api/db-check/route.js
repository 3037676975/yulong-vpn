...
export async function GET() {
  try {
    ...
    const [notices, checks, logs] = await Promise.all([
    ...
    return NextResponse.json({
      ...
      hint: ok ? '后台已经可以读取 Supabase 节点表；如果检测历史/日志表不存在，请运行 v15 数据库升级 SQL。' : '先看 env 两个值是否都是 true；如果是 true 但仍失败，再看 error 字段',
